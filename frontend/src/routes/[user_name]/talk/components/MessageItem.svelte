<script lang="ts">
  ...
  const { message, profile, currentUser, isLoggedIn, onMessageUpdate, onMessageDelete }: Props =
    $props();
...
<div
  class="theme-border theme-visitor-hover mx-2 border-b p-4
...
>
  ...
  <!-- メッセージ内容 -->
  <MessageContent {message} {currentUser} {profile} {isEditingOrDeleting} onDelete={handleDelete} />
