...
<div class="mt-2 flex items-center gap-2 text-xs">
  <button onclick={onReplyClick} class="theme-text-muted transition-colors hover:text-orange-500 dark:hover:text-orange-400">
    💬 返信
  </button>
  ...
  {#if replyCount && replyCount > 0 && onThreadClick}
    <button onclick={onThreadClick} class="theme-text-muted transition-colors hover:text-orange-500 dark:hover:text-orange-400">
      📄 スレッド ({replyCount}件)
    </button>
  {/if}
</div>
