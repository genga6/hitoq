<script lang="ts">
  ...
  const reportTypes: readonly { value: ReportType; label: string }[] = [
    ...
    { value: "inappropriateContent", label: "不適切なコンテンツ" },
...
<Modal {isOpen} title="ユーザーを通報" onClose={handleClose} size="md">
  {#if success}
    <div class="py-4 text-center">
      <div class="theme-text-success mb-2 text-lg font-medium">通報を送信しました</div>
  ...
  {:else}
    <form onsubmit={handleSubmit}>
      ...
      {#if error}
        <div class="theme-text-error mb-4 text-sm">{error}</div>
      {/if}
      ...
      <div class="flex gap-3">
        ...
        <button
          type="submit"
          disabled={loading}
          class="flex-1 rounded-md bg-red-500 px-4 py-2 text-white transition-colors hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-red-600 dark:hover:bg-red-700"
        >
          {loading ? "送信中..." : "通報する"}
        </button>
      </div>
