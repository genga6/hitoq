...
<div
  class="group theme-bg-surface theme-border theme-visitor-hover relative rounded-2xl p-6 transition-all duration-300 {isOwner &&
  answer
    ? 'cursor-pointer'
    : ''}"
  onclick={(e) => {
...
>
  ...
  {#if answer}
    ...
    {/if}
  {:else}
    <p class="theme-text-subtle text-sm italic sm:text-base">回答待ち</p>
  {/if}
  ...
  {#if !isOwner && profileUserId && profileUserName && answer && isLoggedIn && currentUser}
    <div
      class="interaction-buttons absolute top-3 right-3 z-0 flex flex-col items-end gap-1 sm:flex-row sm:items-center sm:gap-1"
    >
      <!-- ハートボタン（いいね） -->
      <button
        onclick={() => {
        ...
        class="theme-text-muted flex items-center gap-1 rounded-full px-2 py-1 text-xs transition-all duration-200 hover:bg-gray-100 hover:text-red-500 sm:px-3 sm:py-1.5 sm:text-sm dark:hover:bg-gray-800"
        title="いいね"
        aria-label="いいね"
      >
        <svg
      ...
      <button
        ...
        class="theme-text-muted flex items-center gap-1 rounded-full px-2 py-1 text-xs transition-all duration-200 hover:bg-gray-100 hover:text-blue-500 sm:px-3 sm:py-1.5 sm:text-sm dark:hover:bg-gray-800"
        title="コメント"
        aria-label="コメントを追加"
      >
