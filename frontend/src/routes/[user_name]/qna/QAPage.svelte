        ...
        {#each availableCategories as categoryId (categoryId)}
          ...
          {#if category}
            <button
              ...
              class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-medium transition-all duration-200 {selectedCategories.includes(
                categoryId
              )
  ...
  {#if filteredAnswerGroups && filteredAnswerGroups.length > 0}
    <div class="relative space-y-6">
      {#each filteredAnswerGroups as group, groupIndex (group.groupId)}
        {@const sameTemplateGroups = filteredAnswerGroups.filter(
          (g) => g.templateId === group.templateId
        )}
        {@const groupNumber =
          sameTemplateGroups.length > 1
            ? sameTemplateGroups.findIndex((g) => g.groupId === group.groupId) + 1
            : null}
        {@const template = availableTemplates.find((t) => t.id === group.templateId)}
        ...
        <QAGroup
          answerGroup={{
            ...group,
            templateTitle: groupNumber
              ? `${group.templateTitle} #${groupNumber}`
              : group.templateTitle
          }}
          {isOwner}
