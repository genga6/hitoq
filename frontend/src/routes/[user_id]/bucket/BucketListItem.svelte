<script lang="ts">
  ...
  const { content, checked, onToggle, onEdit, onDelete, isNew, isOwner }: Props = $props();

  let isEditing = $state(isOwner && (isNew ?? false));
...
<div
  class="group flex items-center justify-between gap-4 p-4 border-b border-gray-200 transition-colors duration-200 {isOwner ? 'hover:bg-orange-50/50' : ''}"
>
    ...
    <div class="flex flex-1 items-center gap-3" transition:fade={{ duration: 150 }}>
      ...
      <button
        ...
        class={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-200
          ...
          ${ isOwner
              ? (checked ? 'hover:bg-orange-600' : 'hover:border-orange-400 hover:text-orange-400')
              : 'cursor-not-allowed'
          }
        `}
