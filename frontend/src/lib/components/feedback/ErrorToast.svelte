...
{#if errorState.isVisible && errorState.errors.length > 0}
  <div class="pointer-events-none fixed inset-0 z-50 flex flex-col items-end justify-start p-6 space-y-4">
    {#each errorState.errors as error (error.id)}
      <div
        class="pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg border shadow-lg {getErrorColors(error.type)}"
        transition:fly={{ x: 300, duration: 300 }}
      >
        <div class="p-4">
          <div class="flex items-start">
            <div class="flex-shrink-0">
              <svg class="h-5 w-5 {getIconColors(error.type)}" viewBox="0 0 20 20" fill="currentColor">
                {#if error.type === 'error'}
              ...
              {#if error.action}
                <div class="mt-3">
                  <button
                    ...
                    class="rounded-md px-3 py-1.5 text-xs font-medium hover:opacity-75 transition-opacity"
            ...
            <div class="ml-4 flex flex-shrink-0">
              <button
                ...
                class="inline-flex rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 hover:opacity-75 transition-opacity"
