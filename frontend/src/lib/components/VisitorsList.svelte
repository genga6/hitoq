  ...
  {#if loading}
    <div class="flex flex-col items-center justify-center py-12 space-y-4">
      <div class="relative">
        ...
        <div class="animate-spin rounded-full h-12 w-12 border-4 border-orange-500 border-t-transparent absolute top-0 left-0"></div>
      </div>
    ...
    {#if filteredVisits.length >= 50}
      <div class="text-center mt-8">
        <div class="inline-flex items-center px-4 py-2 bg-gray-100 rounded-full">
          ...
          <span class="text-sm text-gray-600 font-medium">最新50件まで表示しています</span>
        </div>
      </div>
    {/if}
  {/if}
