  ...
  {:else if (mode === "edit" || mode === "input") && !isEditing}
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center justify-end gap-3">
        {#if secondaryAction}
          <button onclick={handleSkip} class="theme-text-muted px-4 py-2 text-sm hover:opacity-80">
  ...
  {#if showInteractions && !isOwner && profileUserId && profileUserName && answer && isLoggedIn && currentUser}
    ...
    >
      ...
      <button
        ...
        class="flex items-center gap-1 rounded-full px-2 py-1 text-xs transition-all duration-200 hover:bg-gray-100 sm:px-3 sm:py-1.5 sm:text-sm dark:hover:bg-gray-800 {isLiked
          ? 'text-red-500'
          : 'theme-text-muted hover:text-red-500'}"
